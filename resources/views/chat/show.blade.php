<x-app-layout>   
 ...
 <form action="{{ route('messages.create',['id_chat' => $id_chat, 'id_publication' => $id_publication]) }}" method="post">
 @csrf 
   <div class="py-12">
       ...
       <div class="bg-slate-200 dark:bg-red-800 overflow-hidden shadow-sm sm:rounded-lg">
         <div class="p-6 text-decoration-black dark:text-red-100 ">
           <div class="flex justify-between">
             ...
             @if($existingChat)
             <div>
               <h3 class="uppercase text-blue-600 font-bold">{{ $existingChat->name }}</h3>
             </div>
             @endif
             <div>
               <a href="#">
           ...
           <div>
              @if(Auth::check())
                <div class="clearfix" >
                  ...
                  <div class="bg-sky-200 float-right w-3/4 mx-4 my-2 p-2 rounded-lg" type="text" id="remittent" name="remittent" required>
              ...
              @else
                <div class="clearfix">
                  <div class="bg-sky-500 w-3/4 mx-4 my-2 p-2 rounded-lg" type="text" id="addressee" name="addressee" required>
                    <span>{{$message['message']}}</span>
                  </div>
                </div>
              @endif
